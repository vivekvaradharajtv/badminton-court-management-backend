...
router.get('/available-slots', [
    (0, express_validator_1.query)('court_ids').optional().isString(),
    (0, express_validator_1.query)('start_time').optional().matches(TIME_REGEX).withMessage('start_time must be HH:mm'),
    (0, express_validator_1.query)('hours').optional().isFloat({ min: 0.5, max: 24 }).toFloat().withMessage('hours must be 0.5–24'),
], async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        res.status(400).json({
