...
import { deriveBillStatus } from '../utils/billStatus';
...
export async function getDashboardSummary(academyId: string): Promise<DashboardSummary> {
  ...
  for (const b of bills) {
    ...
    if (status !== 'PAID') {
      ...
      else if (status === 'DUE') dueCount += 1;
    }
  }

  const collectionPercentage =
    totalExpected > 0 ? Math.round((totalCollected / totalExpected) * 100 * 100) / 100 : 0;
